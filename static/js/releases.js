...
async function updateReleases() {
    ...
    for (const channel of channels) {
        for (const device of devices) {
            requests.push(fetch(baseUrl + device + "-" + channel)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(text => {
                    ...
                    const factoryFilename =
                        device + "-factory-" + metadata[0] + ".zip";
                    const factoryUrl = baseUrl + factoryFilename;

                    const updateFilename =
                        device + "-ota_update-" + metadata[0] + ".zip";
                    const updateUrl = baseUrl + updateFilename;

                    const tag = metadata[2] + "." + metadata[0];

                    const release =
                        document.getElementById(device + "-" + channel);
                    ...
                    updateLink(links[3], factoryFilename + ".sig",
                        factoryUrl + ".sig");
